...
export async function getBorrowingsByEquipmentId(equipmentId: number) {
  return await db.select().from(Borrowings).where(eq(Borrowings.equipmentId, equipmentId));
}

export async function getBorrowingsByClass(classNumber: number) {
  return await db.select().from(Borrowings).where(eq(Borrowings.class, classNumber));
}
...
export async function createBorrowing(data: { equipmentId: number; class: number; borrowDate: Date; returnDate?: Date }) {
  return await db.insert(Borrowings).values(data);
}

export async function returnBorrowing(id: number, returnDate: Date) {
  return await db.update(Borrowings).set({ returnedAt: returnDate }).where(eq(Borrowings.id, id));
}
